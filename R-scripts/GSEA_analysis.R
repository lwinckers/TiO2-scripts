...
gene_network <- read.table("./data-output/edges_hgnc.txt", header = T, sep = "\t")
...
# save genesets
write.table(genesets, "./data-output/GSEA/genesets.txt", quote = F, row.names = F, sep = "\t")
...
for(i in 4:9) {
  ...
  res <- GSEA(dat, pvalueCutoff = 1,
              ...
              nPerm = 1000)
  loc = paste(getwd(), paste0("/data-output/GSEA/",GOdat,"_GSEA_result",i-3,".txt"),sep="")
