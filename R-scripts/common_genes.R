...
for (i in 1:length(ls_GO)){assign(ls_GO[i], read.table(paste0(path,"/",ls_GO[i]), header = T, sep = "\t")[c("GO.TERM", "SYMBOL")])}
...
GO <- unique(rbind(ann_GO0006915.txt,ann_GO0006954.txt,ann_GO0006974.txt,ann_GO0034599.txt))
colnames(GO) <- c("hgnc_symbol", "entrezgene")

# save common gene list and mapped gene list
write.table(GO ,"./data-output/ann_genes.txt", sep = "\t", col.names = T,
            row.names = F, quote = F)
