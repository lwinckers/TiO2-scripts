...
path = "output/necrosis/"

for(i in go.terms) {
  res <- go_annotations(go.term = i, biomart = biomart, output = paste("output/necrosis/",gsub(":", "", i),".txt", sep=""))
  ...
  file.copy(from = paste("output/necrosis/", gsub(":", "", i),".txt",sep=""), to = paste("data/",gsub(":", "", i),".txt",sep=""), overwrite = TRUE)
}
...
```{r}
path = paste0(getwd(), "/output/necrosis")
...
write.table(GO150, "output/necrosis/GO1.txt", quote = F, sep = "\t", row.names = F)
```
