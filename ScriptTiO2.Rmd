...
# Solution to read pathway ORA results and only select rows where pvalue < 0.05
# Need to rewrite function below to implement this nicely
```{r}
path = paste0(getwd(), "/output")
...
for (i in 1:length(ls_pw)){
filteredPlot(data = ls_pw[[i]], fileName = gsub("ORApw_", "", names(ls_pw[i])))
}
```
