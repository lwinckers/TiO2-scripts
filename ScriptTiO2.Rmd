...
```{r}
go.terms <- c("GO:0097193","GO:0097191")
```
...
for(i in go.terms) {
  res <- go_annotations(go.term = i, biomart = biomart, output = paste("output/ext-int/",gsub(":", "", i),".txt", sep=""))
  ...
  file.copy(from = paste("output/ext-int/", gsub(":", "", i),".txt",sep=""), to = paste("data/",gsub(":", "", i),".txt",sep=""), overwrite = TRUE)
}
...
```{r}
path = paste0(getwd(), "/output/ext-int")
...
write.table(GO150, "output/ext-int/GO1.txt", quote = F, sep = "\t", row.names = F)
write.table(GO250, "output/ext-int/GO2.txt", quote = F, sep = "\t", row.names = F)
```

# Combine the significant results of all four GO-terms
```{r}
sigORA <- rbind(GO150, GO250)
