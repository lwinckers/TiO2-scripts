...
filteredPlot <- function(data, fileName, path) {
  ...
  if (nrow(data) > 0) {
    print(paste0("There are results for ", names(ls_pw[i]), ". A figure will be made."))
    ...
    k <- matrix(0, ncol = 1, nrow = (nrow(nodes)-nrow(data)))
    k <- data.frame(k)
    k$pid5 <- 1
    colnames(k) <- c("pid1", "pid5")
    ...
    print(paste0("There are no results for ", names(ls_pw[i]), ". No figure will be made."))
  }
}